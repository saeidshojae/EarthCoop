...
class StockBidAcceptanceServiceTest extends TestCase
{
    ...
    public function test_secondary_market_active_bahar_bid_is_reserved_before_acceptance(): void
    {
        ...
        $auction=$this->auction(SettlementChannel::ACTIVE_BAHAR,'secondary','holder',$seller->id);
        ...
        $this->assertDatabaseHas('najm_active_bahar_reservations',[
            'reservation_key'=>'accept:1:reserve','amount'=>250,'status'=>ActiveBaharReservation::RESERVED,
        ]);
    ...
    public function test_secondary_market_external_channel_is_rejected_before_bid_or_reservation(): void
    {
        ...
        $seller=User::factory()->create();
        $account=$this->account($user,1000);
        $auction=$this->auction(SettlementChannel::EXTERNAL_USD,'secondary','holder',$seller->id);
    ...
    public function test_seller_cannot_bid_on_own_secondary_listing(): void
    {
        $seller=User::factory()->create();
        $account=$this->account($seller,1000);
        $auction=$this->auction(SettlementChannel::ACTIVE_BAHAR,'secondary','holder',$seller->id);

        $this->expectException(\RuntimeException::class);
        app(StockBidAcceptanceService::class)->acceptActiveBaharBid($seller->id,$account->account_number,$auction,25,1,'accept:self');
    }

    public function test_base_price_is_always_a_canonical_bid_floor(): void
    {
        $user=User::factory()->create();
        $account=$this->account($user,1000);
        $auction=$this->auction(SettlementChannel::ACTIVE_BAHAR,'primary','treasury');
        $auction->update(['base_price_gol'=>25,'min_bid_gol'=>null]);

        $this->expectException(\RuntimeException::class);
        app(StockBidAcceptanceService::class)->acceptActiveBaharBid($user->id,$account->account_number,$auction->fresh(),24,1,'accept:floor');
    }
    ...
    private function auction(string $channel,string $market,string $supply,?int $sellerUserId=null): Auction
    {
        ...
        return Auction::create([
            'stock_id'=>$stock->id,'seller_user_id'=>$sellerUserId,
            'seller_holding_reservation_key'=>$market==='secondary'?'seller:test:reserved':null,
            'market_type'=>$market,'supply_source'=>$supply,'settlement_channel'=>$channel,'quote_unit'=>'gol',
            'shares_count'=>100,'base_price'=>1,'base_price_gol'=>10,'min_bid_gol'=>10,'max_bid_gol'=>100,
            'start_time'=>now()->subMinute(),'ends_at'=>now()->addHour(),'status'=>'running','type'=>'uniform_price','lot_size'=>100,
        ]);
    }
}
