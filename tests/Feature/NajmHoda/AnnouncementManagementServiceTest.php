...
class AnnouncementManagementServiceTest extends TestCase
{
    ...
    {
        $actor=$this->user();
        $group=Group::query()->create(['name'=>'Announcement group '.uniqid('',true),'is_open'=>1,'location_level'=>'7']);
    ...
    public function test_low_risk_notification_action_creates_draft_without_publishing(): void
    {
        ...
        $result=app(FounderLowRiskDomainActionService::class)->execute('notifications','draft_announcement',[
            'title'=>'اعلان پیشنهادی','content'=>'هنوز منتشر نشده','group_level'=>'7','should_pin'=>false,
            'reason_code'=>'announcement-test-'.uniqid(),'requested_by'=>$actor->id,
        ]);

        $this->assertTrue($result['success']);
        $this->assertSame('drafted',$result['status']);
        $this->assertDatabaseHas('founder_announcement_drafts',['id'=>$result['draft_id'],'status'=>'draft']);
        $this->assertSame(0,Announcement::query()->where('title','اعلان پیشنهادی')->count());
    }

    private function user(): User
    {
        return User::query()->create([
            'email'=>uniqid('announcement-',true).'@example.test','password'=>Hash::make('password'),
            'status'=>1,'first_name'=>'Test','last_name'=>'Actor','is_system'=>false,
        ]);
    }
}
