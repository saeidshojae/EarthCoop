...
async function groupChatFetch(input, init = {}) {
    ...
    function updateLastMessageCursor(messageId) {
        ...
        if (numericId && (!lastMessageId || numericId > lastMessageId)) lastMessageId = numericId;
    }
    function updateLastPostCursor(postId) {
        const numericId = parseInt(postId, 10);
        if (numericId && numericId > lastPostId) lastPostId = numericId;
    }
    ...
    const applyFeedItemThroughPipeline = (type, action, payload, source) => action === 'create'
