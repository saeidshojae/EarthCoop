    ...
    public function root(Request $request): JsonResponse
    {
        ...
        $hasIranV2 = $iranV2RuntimeEnabled
            && Location::query()
                ...
                ->exists();
        $preferIranV2 = $countryCode === 'IR' && $hasIranV2;
        ...
        $locations = Location::query()->with(['type', 'schema'])->whereNull('parent_id')->where('status', 'active')
            ...
            ->whereHas('schema', function ($query) use ($preferIranV2, $iranV2RuntimeEnabled, $countryCode, $hasIranV2): void {
                ...
                if ($preferIranV2) {
                    ...
                    $query->where(fn ($candidate) => $candidate
                        ...
                        ->orWhere('version', '!=', 'v2'));
                } elseif ($countryCode === '' && $hasIranV2) {
                    $query->where(fn ($candidate) => $candidate
                        ->where('country_code', '!=', 'IR')
                        ->orWhere(fn ($iran) => $iran
                            ->where('country_code', 'IR')
                            ->where('key', 'ir-reference-v2')
                            ->where('version', 'v2')));
                }
