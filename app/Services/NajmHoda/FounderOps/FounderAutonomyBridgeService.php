    ...
    public function plan(int $hours = 24, int $limit = 12): array
    {
        ...
        foreach ($this->moderationCandidates->candidates(min(5, $limit)) as $candidate) {
            ...
            foreach (['classify_report','prepare_case_summary'] as $action) {
                $context = [
                    'entity_type'=>$sourceType,'entity_id'=>$sourceId,
                    'attention_priority'=>in_array(($candidate['priority'] ?? ''), ['critical','high'], true) ? 'P1' : 'P2',
                    'reason_code'=>substr(hash('sha256','moderation|'.$action.'|'.$sourceType.'|'.$sourceId),0,20),
                    'source_event'=>'founder_moderation_candidate',
                ];
                $prepared[] = [
                    'source_attention'=>['priority'=>$context['attention_priority'],'domain'=>'reports_moderation','title'=>'Moderation report requires review'],
                    'domain'=>'reports_moderation','action'=>$action,'action_context'=>$context,
                    'preparation'=>$this->requests->prepare('reports_moderation',$action,$context),
                ];
            }
        }

        foreach ($items as $item) {
            if (! is_array($item) || in_array((string)($item['domain'] ?? ''), ['support','reports_moderation'], true)) continue;
            $mapped = $this->mapAttentionToAction($item);
            if ($mapped === null) continue;
            [$domain,$action] = $mapped;
            $context = [
                'attention_priority'=>(string)($item['priority'] ?? ''),
                'reason_code'=>$this->reasonCode($item),'source_event'=>'founder_attention',
            ];
