...
class FounderAttentionService
{
    ...
    public function brief(int $hours = 24): array
    {
        ...
        $sensitiveConfigChanges = collect((array) data_get($snapshot, 'recent_managed_events', []))
            ...
            ->count();
        if ($sensitiveConfigChanges > 0) {
            $items[] = $this->item('P2', 'admin_settings', 'Sensitive admin configuration changed in the reporting window', ['events' => $sensitiveConfigChanges]);
        }
        ...
        if ($announcements > 0) $items[] = $this->item('P3', 'notifications', 'Announcements were published in the reporting window', [
            'count' => $announcements,
            'pinned' => (int) data_get($snapshot, 'notifications.pinned_announcements_in_window', 0),
        ]);
