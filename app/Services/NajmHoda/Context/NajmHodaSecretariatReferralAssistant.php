...
class NajmHodaSecretariatReferralAssistant
{
    ...
    public function intercept(User $actor, array $pageContext, string $message, ?int $conversationId = null): ?array
    {
        ...
        if (! $this->looksLikeReferral($message)) {
            return null;
        }

        $targetUserId = $this->extractId($message, ['کاربر', 'گیرنده', 'user']);
        if ($targetUserId <= 0) {
            return $this->response('برای ارجاع داخلی، شناسه کاربر گیرنده را مشخص کنید؛ مثال: «ارجاع بده | کاربر: 42 | دستور: بررسی و اعلام نظر».', 'needs_input');
        }
        ...
        if (! $target || ! $this->isGroupMember((int) $record->office->scope_id, $targetUserId)) {
            return $this->response('کاربر گیرنده عضو گروه این دفتر نیست.', 'blocked');
        }
        ...
        if ($displayName === '') {
            $displayName = (string) ($target->email ?? ('User #' . $target->id));
        }
    ...
    private function confirm(User $actor, int $recordId, array $pending): array
    {
        ...
        if ($record->office?->scope_type !== 'group'
            ...
            return $this->response('گیرنده دیگر برای این دفتر معتبر نیست؛ هیچ ارجاعی ایجاد نشد.', 'stale_preview');
        }
    ...
    {
        foreach ($labels as $label) {
            if (preg_match('/(?:^|[|؛;])\s*' . preg_quote($label, '/') . '\s*[:：]\s*(.*?)(?=\s*[|؛;]\s*[^|؛;]+\s*[:：]|$)/us', $message, $match)) {
