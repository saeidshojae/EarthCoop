 ...
 *
 * Source-domain IDs are hints only. Preview re-resolves approved/adopted source
 * rows in the Office group, checks Secretariat create authority, and persists
 * nothing. Explicit human confirmation delegates to S3 which locks/revalidates
 * source provenance and creates/reuses a Draft only.
 */
class NajmHodaSecretariatGovernanceDraftAssistant
{
    ...
    public function intercept(User $actor, array $pageContext, string $message, ?int $conversationId = null): ?array
    {
        ...
        if ($this->isMinuteConfirmation($message) && is_array($pending = Cache::get($minuteKey))) {
            Cache::forget($minuteKey);
            return $this->saveMinute($actor, $office, $pending);
        }
        if ($this->isResolutionConfirmation($message) && is_array($pending = Cache::get($resolutionKey))) {
            Cache::forget($resolutionKey);
            return $this->saveResolution($actor, $office, $pending);
        }

        if ($this->looksLikeMinuteRequest($message)) {
            $minuteId = $this->extractId($message, ['صورتجلسه', 'minute']);
            if ($minuteId <= 0) {
                return $this->response('شناسه صورتجلسه تأییدشده را مشخص کنید؛ مثال: «صورتجلسه رسمی آماده کن | صورتجلسه: 12».', 'needs_input');
            }
        ...
        if ($this->looksLikeResolutionRequest($message)) {
            ...
            if ($resolutionId <= 0) {
                return $this->response('شناسه Governance Resolution را مشخص کنید؛ مثال: «مصوبه رسمی آماده کن | مصوبه: 20».', 'needs_input');
            }
    ...
    private function previewMinute(User $actor, SecretariatOffice $office, int $minuteId, string $key): array
    {
        ...
        if (! $minute
            || (int) $minute->group_id !== (int) $office->scope_id
            || $minute->status !== 'approved'
            || $minute->approved_by === null
            || $minute->approved_at === null
            || $minute->session === null) {
            return $this->response('صورتجلسه تأییدشده و سازگار با این دفتر پیدا نشد.', 'blocked');
        }

        if (! $this->canCreate($actor, $office, 'meeting_minute')) {
            return $this->response('شما مجوز ایجاد Draft صورتجلسه در این دفتر را ندارید.', 'blocked');
        }
        ...
        Cache::put($key, [
            ...
            'source_updated_at' => optional($minute->updated_at)->toJSON(),
        ], now()->addMinutes(15));
        ...
        return $this->response(
            ...
            ['source_type' => 'meeting_minute', 'source_id' => (int) $minute->id]
        );
    }
    ...
    private function previewResolution(User $actor, SecretariatOffice $office, int $resolutionId, int $minuteRecordId, string $key): array
    {
        ...
        if (! $resolution
            || (int) $resolution->group_id !== (int) $office->scope_id
            || $resolution->status !== 'adopted'
            || $resolution->adopted_at === null
            || $resolution->proposal === null) {
            return $this->response('مصوبه adopted و سازگار با این دفتر پیدا نشد.', 'blocked');
        }

        if (! $this->canCreate($actor, $office, 'resolution')) {
            return $this->response('شما مجوز ایجاد Draft مصوبه در این دفتر را ندارید.', 'blocked');
        }
        ...
        if ($minuteRecordId > 0) {
            ...
            if (! $minuteRecord
                || (int) $minuteRecord->office_id !== (int) $office->id
                || $minuteRecord->record_type !== 'meeting_minute'
                || ! Gate::forUser($actor)->allows('view', $minuteRecord)) {
        ...
        Cache::put($key, [
            ...
            'source_updated_at' => optional($resolution->updated_at)->toJSON(),
            'minute_record_id' => $minuteRecord?->id,
            'minute_record_version_id' => $minuteRecord?->current_version_id,
        ], now()->addMinutes(15));

        $lines = [
            'پیش‌نمایش ثبت مصوبه در دبیرخانه:',
            'عنوان: ' . (string) $resolution->proposal->title,
            'خلاصه: ' . mb_substr((string) ($resolution->proposal->summary ?? ''), 0, 1000),
            'شرح مصوبه: ' . mb_substr((string) ($resolution->proposal->description ?? ''), 0, 1600),
            'زمان تصویب: ' . (string) optional($resolution->adopted_at)->toIso8601String(),
        ];
        if ($minuteRecord) {
            $lines[] = 'رابطه پیشنهادی decision_of با رکورد صورتجلسه #' . $minuteRecord->id;
        }
        $lines[] = '';
        $lines[] = 'برای ساخت Draft دبیرخانه «ذخیره مصوبه» بفرستید.';
    ...
    {
        if (! $this->validPending($actor, $office, $pending)) {
        ...
        if (! $minute
            || (string) optional($minute->updated_at)->toJSON() !== (string) ($pending['source_updated_at'] ?? '')
            || ! $this->canCreate($actor, $office, 'meeting_minute')) {
        ...
        return $this->response(
            "Draft صورتجلسه دبیرخانه ایجاد شد (Draft #{$record->id}). ثبت رسمی هنوز انجام نشده است.",
            'draft_saved',
    ...
    private function saveResolution(User $actor, SecretariatOffice $office, array $pending): array
    {
        ...
        if (! $resolution
            || (string) optional($resolution->updated_at)->toJSON() !== (string) ($pending['source_updated_at'] ?? '')
            || ! $this->canCreate($actor, $office, 'resolution')) {
        ...
        if ((int) ($pending['minute_record_id'] ?? 0) > 0) {
            ...
            if (! $minuteRecord
                || (int) $minuteRecord->office_id !== (int) $office->id
                || $minuteRecord->record_type !== 'meeting_minute'
                || (int) $minuteRecord->current_version_id !== (int) ($pending['minute_record_version_id'] ?? 0)
                || ! Gate::forUser($actor)->allows('view', $minuteRecord)) {
        ...
        return $this->response(
            "Draft مصوبه دبیرخانه ایجاد شد (Draft #{$record->id}). ثبت رسمی هنوز انجام نشده است.",
            'draft_saved',
            ['record_id' => (int) $record->id, 'record_status' => (string) $record->status]
        );
    }
    ...
    {
        return (int) ($pending['actor_id'] ?? 0) === (int) $actor->id
            && (int) ($pending['office_id'] ?? 0) === (int) $office->id
            && (int) ($pending['source_id'] ?? 0) > 0;
    }
    ...
    private function extractId(string $message, array $labels): int
    {
        foreach ($labels as $label) {
            if (preg_match('/' . preg_quote($label, '/') . '\s*[:：]\s*(\d+)/iu', $message, $match)) {
                return (int) $match[1];
            }
        }
        return 0;
    ...
    private function response(string $message, string $status, array $extra = []): array
    {
        return array_merge([
            'success' => true,
            'message' => $message,
            'agent' => 'secretariat_governance_draft',
            'agent_name' => 'نجم‌هدا',
            'agent_icon' => '✦',
            'suggestions' => [],
            'grounded' => true,
