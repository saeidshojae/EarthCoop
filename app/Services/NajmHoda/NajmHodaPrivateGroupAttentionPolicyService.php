...
class NajmHodaPrivateGroupAttentionPolicyService
{
    public function intercept(User $user, array $ctx, string $message, ?int $conversationId = null): ?array
    {
        ...
        if (is_array($pending) && $this->isCancel($message)) {
            Cache::forget($key);
            return $this->reply('تغییر تنظیمات پیگیری لغو شد و چیزی تغییر نکرد.', 'cancelled');
        }
        if (is_array($pending) && $this->isConfirm($message)) {
            if (! $this->leader($groupId, $user->id)) {
                Cache::forget($key);
                return $this->reply('مجوز تغییر تنظیمات پیگیری این گروه را ندارید.', 'blocked');
            }
            Cache::forget($key);
            $setting = $this->setting($groupId);
            $setting->fill((array) ($pending['changes'] ?? []))->save();
        ...
        if (! $this->leader($groupId, $user->id)) {
            return $this->reply('تنظیمات پیگیری فعال فقط برای مدیران و بازرسان فعال گروه قابل تغییر است.', 'blocked');
        }

        $setting = $this->setting($groupId);
        $changes = $this->changes($message);
        if ($changes === []) {
            return $this->reply("تنظیمات فعلی پیگیری نجم هدا برای «{$group->name}»:\n\n" . $this->status($setting), 'policy_status');
        }

        Cache::put($key, ['changes' => $changes], now()->addMinutes(20));
        $lines = ["تغییر پیشنهادی تنظیمات پیگیری نجم هدا برای «{$group->name}»:", ''];
        foreach ($changes as $field => $value) $lines[] = '• ' . $this->label($field, $value);
        $lines[] = '';
        $lines[] = 'تا قبل از تأیید چیزی تغییر نمی‌کند. برای اعمال «تأیید» و برای انصراف «لغو» بفرستید.';
        return $this->reply(implode("\n", $lines), 'awaiting_confirmation');
    }

    protected function setting(int $groupId): NajmHodaGroupAttentionSetting
    {
        return NajmHodaGroupAttentionSetting::firstOrCreate(['group_id' => $groupId], [
            'enabled' => false, 'due_soon_hours' => 48, 'suppress_minutes' => 720,
            'digest_mode' => 'daily', 'timezone' => (string) config('app.timezone', 'UTC'), 'preferred_time' => '08:00',
        ]);
    }
    ...
    protected function changes(string $message): array
    {
        ...
        if ($this->any($m, ['پیگیری فعال را روشن','پیگیری نجم هدا را روشن','هشدارها را روشن'])) $c['enabled'] = true;
        if ($this->any($m, ['پیگیری فعال را خاموش','پیگیری نجم هدا را خاموش','هشدارها را خاموش'])) $c['enabled'] = false;
        if ($this->any($m, ['هشدارها را فوری','هشدار فوری','اعلان فوری','فوری خبر بده'])) { $c['enabled'] = true; $c['digest_mode'] = 'immediate'; }
    ...
    protected function looksRelevant(string $message): bool
    {
        return $this->any($this->norm($message), ['تنظیمات پیگیری','پیگیری فعال','هشدار نجم هدا','هشدارهای نجم هدا','خلاصه روزانه','گزارش روزانه','هشدار فوری','اعلان فوری','موعد نزدیک','نزدیک موعد','تکرار هشدار','فاصله هشدار']);
    }

    protected function status(NajmHodaGroupAttentionSetting $s): string
    {
        $mode = match ((string)$s->digest_mode) { 'immediate'=>'فوری','daily'=>'خلاصه روزانه','off'=>'فقط ثبت داخلی؛ بدون اعلان',default=>(string)$s->digest_mode };
        return "• پیگیری فعال: " . ($s->enabled?'روشن':'خاموش') . "\n• حالت اعلان: {$mode}\n• ساعت خلاصه: {$s->preferred_time}\n• نزدیک موعد: {$s->due_soon_hours} ساعت\n• فاصله تکرار: {$s->suppress_minutes} دقیقه";
    }

    protected function label(string $f, mixed $v): string
    {
        return match ($f) {
            'enabled'=>'پیگیری فعال → '.($v?'روشن':'خاموش'),
            'digest_mode'=>'حالت اعلان → '.match($v){'immediate'=>'فوری','daily'=>'خلاصه روزانه','off'=>'فقط ثبت داخلی؛ بدون اعلان',default=>(string)$v},
            'preferred_time'=>'ساعت خلاصه → '.$v,
            'due_soon_hours'=>'بازه نزدیک موعد → '.$v.' ساعت',
            'suppress_minutes'=>'فاصله تکرار → '.$v.' دقیقه',
            default=>$f.' → '.(string)$v,
        };
    }

    protected function leader(int $groupId, int $userId): bool
    {
        $role = GroupUser::where('group_id',$groupId)->where('user_id',$userId)->where('status',1)->value('role');
        return in_array((int)$role,[2,3],true);
    }

    protected function norm(string $v): string
    {
        $v = mb_strtolower(trim(strip_tags($v)));
        $v = str_replace(['ي','ك','ۀ'],['ی','ک','ه'],$v);
