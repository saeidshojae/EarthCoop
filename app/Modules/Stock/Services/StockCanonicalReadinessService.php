...
class StockCanonicalReadinessService
{
    public function audit(): array
    {
        ...
        $capitalRequired=$canonical->contains(fn(Auction $a): bool => (string)($a->stock?->issuer_type??'')===SettlementEligibilityPolicy::ISSUER_EARTHCOOP && (string)$a->market_type===SettlementEligibilityPolicy::MARKET_PRIMARY && (string)$a->supply_source===SettlementEligibilityPolicy::SUPPLY_TREASURY && (string)$a->settlement_channel===SettlementChannel::ACTIVE_BAHAR);
        ...
        $checks['earthcoop_capital_account_required']=$capitalRequired; $checks['earthcoop_capital_account']=$capitalOk;
        ...
        foreach($canonical as $auction){
            try { $auction->assertSettlementEligible(); } catch(\Throwable $e){ $blockers[]=$this->item('auction_boundary_invalid','Canonical auction fails settlement eligibility.',['auction_id'=>$auction->id]); }
            if(strtolower((string)$auction->quote_unit)!=='gol'||(int)$auction->base_price_gol<=0) $blockers[]=$this->item('canonical_pricing_invalid','Canonical auction lacks valid Gol pricing.',['auction_id'=>$auction->id]);
            ...
            $issuer=(string)($auction->stock?->issuer_type??''); $market=(string)$auction->market_type; $supply=(string)$auction->supply_source; $channel=(string)$auction->settlement_channel;
            ...
            if(in_array($channel,[SettlementChannel::EXTERNAL_IRR,SettlementChannel::EXTERNAL_USD],true)&&!config('stock.external_capital_enabled',false)) $blockers[]=$this->item('external_capital_not_configured','External capital provider/rate-source cutover is disabled.',['auction_id'=>$auction->id,'channel'=>$channel]);
            if($issuer===SettlementEligibilityPolicy::ISSUER_PROJECT&&$market===SettlementEligibilityPolicy::MARKET_PRIMARY&&$supply===SettlementEligibilityPolicy::SUPPLY_TREASURY&&$channel===SettlementChannel::ACTIVE_BAHAR) $blockers[]=$this->item('project_payee_mapping_missing','Project primary Active Bahar settlement has no canonical project payee-account mapping yet.',['auction_id'=>$auction->id,'stock_id'=>$auction->stock_id,'issuer_id'=>$auction->stock?->issuer_id]);
        }

        $reconciliation=StockSettlementAllocation::query()->where('state',StockSettlementAllocation::RECONCILIATION_REQUIRED)->count();
        $checks['reconciliation_required']=$reconciliation; if($reconciliation>0) $blockers[]=$this->item('settlement_reconciliation_required','Confirmed money exists with incomplete Stock allocation.',['count'=>$reconciliation]);
        ...
        $checks['orphan_stock_bid_reservations']=$orphanReservations; if($orphanReservations>0) $blockers[]=$this->item('orphan_stock_bid_reservation','Open Active Bahar reservation has no canonical Bid.',['count'=>$orphanReservations]);
        ...
        $checks['orphan_secondary_share_reservations']=$orphanSellerReservations; if($orphanSellerReservations>0) $blockers[]=$this->item('orphan_secondary_share_reservation','Open seller share reservation has no matching secondary auction.',['count'=>$orphanSellerReservations]);
        ...
        $checks['expired_running_canonical_auctions']=$expiredActive; if($expiredActive>0) $warnings[]=$this->item('expired_running_auction','Canonical auctions have expired but remain running.',['count'=>$expiredActive]);
