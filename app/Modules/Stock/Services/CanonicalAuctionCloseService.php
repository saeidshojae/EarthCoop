...
class CanonicalAuctionCloseService
{
    ...
    public function close(Auction $auction): array
    {
        ...
        if (! $auction->hasCanonicalGolPricing()) {
            throw new RuntimeException('Canonical close requires Gol pricing.');
        }
        if ((string)$auction->settlement_channel !== SettlementChannel::ACTIVE_BAHAR) {
            throw new RuntimeException('Canonical close currently supports Active Bahar only.');
        }
        if ((string)$auction->market_type !== SettlementEligibilityPolicy::MARKET_PRIMARY
        ...
        if ($payeeAccountNumber === '') {
            throw new RuntimeException('EarthCoop capital Najm Bahar account is not configured.');
        }
        if (! Account::query()->where('account_number',$payeeAccountNumber)->where('status',1)->exists()) {
            throw new RuntimeException('Configured EarthCoop capital Najm Bahar account does not exist or is inactive.');
        }
        ...
        return DB::transaction(function () use ($auction,$payeeAccountNumber) {
            ...
            if ($auction->status === 'settled') {
                return ['status'=>'settled','auction_id'=>$auction->id,'idempotent'=>true];
            }
            if (! in_array((string)$auction->status,['running','settling'],true)) {
                throw new RuntimeException('Auction is not in a canonical closable state.');
            }
            ...
            $bids=Bid::query()
                ...
                ->where('status','active')
                ->whereNotNull('price_gol')
                ->whereNotNull('reservation_key')
                ->orderByDesc('price_gol')
                ->orderBy('created_at')
                ->orderBy('id')
                ->lockForUpdate()
                ->get();
            ...
            foreach ($plan['allocations'] as $item) {
                ...
                $settled[]=[
                    ...
                    'price_gol'=>$item['price_gol'],
                    'total_gol'=>$item['quantity']*$item['price_gol'],
                ];
            }
            ...
            return [
                ...
                'winners'=>$settled,
                'allocated_shares'=>array_sum(array_column($settled,'quantity')),
                'clearing_price_gol'=>$plan['clearing_price_gol'],
            ];
        });
    }
    ...
    private function buildPlan(Auction $auction,$bids,int $capacity): array
    {
        ...
        if ($type==='single_winner') {
            $bid=$bids->first();
            $qty=min((int)$bid->quantity,$remaining);
        ...
        if (! in_array($type,['uniform_price','pay_as_bid'],true)) {
            throw new RuntimeException('Unsupported canonical auction type.');
        }
        ...
        foreach ($bids as $bid) {
            ...
            $qty=min((int)$bid->quantity,$remaining);
            if ($qty<=0) continue;
            $allocations[]=['bid_id'=>$bid->id,'quantity'=>$qty,'price_gol'=>(int)$bid->price_gol];
            $remaining-=$qty;
        }

        $clearing=$allocations!==[] ? (int)end($allocations)['price_gol'] : null;
        if ($type==='uniform_price' && $clearing!==null) {
            foreach ($allocations as &$allocation) $allocation['price_gol']=$clearing;
            unset($allocation);
        }
