...
class SecondaryListingService
{
    ...
    public function create(
        ...
        if (! config('stock.secondary_market_enabled', false)) {
            throw new RuntimeException('Secondary market is not enabled.');
        }
        if ($sellerUserId <= 0 || $stockId <= 0 || $quantity <= 0 || $basePriceGol <= 0) {
            throw new InvalidArgumentException('Seller, stock, quantity and base price must be positive.');
        }
        if (trim($listingKey) === '') throw new InvalidArgumentException('Listing key is required.');
        if (! in_array($auctionType, ['single_winner','uniform_price','pay_as_bid'], true)) {
            throw new InvalidArgumentException('Unsupported secondary auction type.');
        }
        ...
        if ($minBidGol !== null && $maxBidGol !== null && $minBidGol > $maxBidGol) {
            throw new InvalidArgumentException('Minimum bid cannot exceed maximum bid.');
        }
        ...
        return DB::transaction(function () use ($sellerUserId,$stockId,$quantity,$basePriceGol,$minBidGol,$maxBidGol,$auctionType,$endsAt,$listingKey,$info) {
            $existing = Auction::query()->where('seller_user_id',$sellerUserId)
                ->where('stock_id',$stockId)
                ->where('info','like','%listing_key='.$listingKey.'%')
                ->lockForUpdate()->first();
            ...
            $auction = Auction::query()->create([
                ...
                'base_price'=>0,
                'base_price_gol'=>$basePriceGol,
                'min_bid'=>0,
                'min_bid_gol'=>$minBidGol,
                'max_bid'=>0,
                'max_bid_gol'=>$maxBidGol,
                'lot_size'=>$quantity,
                'start_time'=>now(),
                'ends_at'=>$endsAt,
                'status'=>'running',
                'type'=>$auctionType,
                'info'=>trim(($info ? $info.' | ' : '').'listing_key='.$listingKey),
            ]);

            $reservationKey='secondary:listing:'.$listingKey.':shares';
            return $this->supply->attachSellerSupply($auction,$sellerUserId,$quantity,$reservationKey);
        });
    }
