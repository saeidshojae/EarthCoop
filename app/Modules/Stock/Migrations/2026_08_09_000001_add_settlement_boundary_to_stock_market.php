...
return new class extends Migration
{
    ...
    {
        // Upgrade path for databases created before issuer metadata became
        // canonical. Fresh schemas already contain these columns/indexes.
        if (!Schema::hasColumn('stocks', 'issuer_type')) {
            Schema::table('stocks', function (Blueprint $table) {
                // Intentionally nullable for legacy rows. External settlement
                // must fail closed until an issuer is explicitly classified.
        ...
        // Same dual-path rule for auctions: fresh installs get the canonical
        // boundary from create_auctions; older databases are upgraded here.
        if (!Schema::hasColumn('auctions', 'market_type')) {
