    ...
    <div class="card shadow-sm border-0 rounded-4">
        <div class="card-body p-4">
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-4"><div class="border rounded-3 p-3 h-100"><div class="text-muted small mb-1">کل سهام شما</div><div class="fs-4 fw-bold">{{ number_format($holding->quantity) }}</div></div></div>
                <div class="col-md-4"><div class="border rounded-3 p-3 h-100"><div class="text-muted small mb-1">قابل عرضه</div><div class="fs-4 fw-bold">{{ number_format($availableQuantity) }}</div></div></div>
                <div class="col-md-4"><div class="border rounded-3 p-3 h-100"><div class="text-muted small mb-1">واحد قیمت</div><div class="fs-5 fw-bold">Gol / Active Bahar</div></div></div>
            </div>

            <form method="POST" action="{{ route('stock.secondary-listing.store', $holding) }}">
                @csrf
                <input type="hidden" name="listing_key" value="{{ old('listing_key', $listingKey) }}">
                <div class="row g-3">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6"><label class="form-label">حداقل پیشنهاد (Gol)</label><input class="form-control" type="number" name="min_bid_gol" min="1" value="{{ old('min_bid_gol') }}" @disabled(!$secondaryEnabled)></div>
                    <div class="col-md-6"><label class="form-label">حداکثر پیشنهاد (Gol)</label><input class="form-control" type="number" name="max_bid_gol" min="1" value="{{ old('max_bid_gol') }}" @disabled(!$secondaryEnabled)></div>
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6"><label class="form-label">مدت حراج (ساعت)</label><input class="form-control" type="number" name="duration_hours" min="1" max="720" value="{{ old('duration_hours',24) }}" required @disabled(!$secondaryEnabled)></div>
                    <div class="col-12"><label class="form-label">توضیحات اختیاری</label><textarea class="form-control" name="info" rows="3" maxlength="1000" @disabled(!$secondaryEnabled)>{{ old('info') }}</textarea></div>
                </div>
                ...
                <button type="submit" class="btn btn-success px-4" @disabled(!$secondaryEnabled || $availableQuantity < 1)>ایجاد عرضه ثانویه</button>
            </form>
        </div>
    </div>
</div>
