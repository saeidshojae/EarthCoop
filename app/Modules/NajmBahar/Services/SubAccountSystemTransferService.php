...
/**
 * Canonical boundary for Active money leaving a member sub-account to a
 * system account.
 *
 * Release C deliberately preserves the current legacy parent semantics: the
 * main user Account.balance may still represent aggregate wealth, so an
 * external child spend reduces that stored aggregate total while the
 * parent's local active/dim fields are left untouched. This compatibility
 * rule is isolated here instead of remaining embedded in TransactionService.
 */
class SubAccountSystemTransferService
{
    public function transfer(
        ...
        return DB::transaction(function () use (
            ...
            if ((int) $lockedSource->status !== 1) {
                throw new \RuntimeException('Sub-account is inactive');
            }

            $sourceMirror = Account::query()
                ->where('account_number', $lockedSource->sub_account_code)
            ...
            // Preserve the still-supported legacy aggregate-total parent model.
            $parent->balance = (int) ($parent->balance ?? 0) - $amount;
            $parent->save();

            $meta = array_merge($metadata, [
                'balance_type' => 'active',
                'routed_by' => 'sub_account_system_transfer_service',
                'from_sub_account_id' => (int) $lockedSource->id,
            ]);
            if ($transactionType) {
            ...
            $transaction = NajmTransaction::create([
                'from_account_id' => (int) $sourceMirror->id,
            ...
            LedgerEntry::create([
                ...
                'account_id' => (int) $sourceMirror->id,
                'amount' => -$amount,
                'entry_type' => 'debit',
                'meta' => $meta,
            ]);
