                            ...
                            <div class="col-md-6">  
                                <select id="continent" class="form-control @error('continent') is-invalid @enderror" name="continent" required>  
                                    @foreach($continents as $continent)  
                                        <option value="{{ $continent->id }}" {{ old('continent', $user->continent_id) == $continent->id ? 'selected' : '' }}>{{ $continent->name_en }}</option>  <!-- اصلاح این خط -->  
                                    @endforeach  
                                </select>  
                                @error('continent')  
                            ...
                            <div class="col-md-6">  
                                <select id="country" class="form-control @error('country') is-invalid @enderror" name="country" required>  
                                    @foreach($countries as $country)  
                                        <option value="{{ $country->id }}" {{ old('country', $user->country_id) == $country->id ? 'selected' : '' }}>{{ $country->name_en }}</option>  <!-- اصلاح این خط -->  
                                    @endforeach  
                                </select>  
                                @error('country')  
                                    <span class="invalid-feedback" role="alert">  
