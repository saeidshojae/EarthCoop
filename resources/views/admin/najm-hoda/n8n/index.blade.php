...
<div class="container-fluid py-3" dir="rtl">
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-md-3"><div class="card h-100 shadow-sm"><div class="card-body"><div class="text-muted small">Cache</div><div class="fs-5 fw-bold">{{ $report['cache_driver'] }}</div><div class="small {{ $report['checks']['persistent_cache'] ? 'text-success' : 'text-danger' }}">{{ $report['checks']['persistent_cache'] ? 'اشتراکی/پایدار' : 'برای callback واقعی کافی نیست' }}</div></div></div></div>
        <div class="col-md-3"><div class="card h-100 shadow-sm"><div class="card-body"><div class="text-muted small">Secret</div><div class="fs-5 fw-bold">{{ $report['secret_configured'] ? 'تنظیم شده' : 'تنظیم نشده' }}</div><div class="small text-muted">مقدار secret هرگز در پنل نمایش داده نمی‌شود.</div></div></div></div>
        <div class="col-md-3"><div class="card h-100 shadow-sm"><div class="card-body"><div class="text-muted small">Callbackها</div><div class="fs-4 fw-bold">{{ number_format($counts['total']) }}</div><div class="small text-muted">موفق: {{ $counts['completed'] }} | خطا: {{ $counts['failed'] }}</div></div></div></div>
    </div>
