    ...
    <div class="nh-chat-shell">
        ...
        <div id="ministry" class="nh-pane active">
            <div class="nh-ministry">
                <div class="d-flex flex-wrap align-items-center justify-content-between gap-2">
                    <div>
                        ...
                        <div class="text-muted small">گزارش استثنامحور و تصمیم‌پذیر روی Founder Ops واقعی؛ بدون مسیر اجرایی موازی.</div>
                    </div>
                    <div class="nh-toolbar">
                        <select id="windowSelect" class="form-select form-select-sm nh-window">
                ...
                <div class="nh-ministry-group">
                    <div class="nh-ministry-group-title">فرمان‌های روزانه</div>
                    <div class="nh-ministry-actions">
                        <button class="nh-intent" data-intent="morning_brief">☀ صبح مدیرکل</button>
                        <button class="nh-intent" data-intent="urgent_items">⚠ کارهای فوری من</button>
                        <button class="nh-intent" data-intent="pending_approvals">✓ در انتظار تأیید من</button>
                        <button class="nh-intent" data-intent="communications">✉ ارتباطات</button>
                        <button class="nh-intent" data-intent="system_health">♡ سلامت سامانه</button>
                        <button class="nh-intent" data-intent="end_of_day">☾ پایان روز مدیرکل</button>
                    </div>
                </div>

                <div class="nh-ministry-group">
                    <div class="nh-ministry-group-title">حوزه‌های مدیریتی</div>
                    <div class="nh-ministry-actions">
                        <button class="nh-intent domain" data-intent="users_registration">👤 کاربران و ثبت‌نام</button>
                        <button class="nh-intent domain" data-intent="reference_data">⌖ مکان / صنف / تخصص</button>
                        <button class="nh-intent domain" data-intent="support_moderation">☏ پشتیبانی و شکایات</button>
                        <button class="nh-intent domain" data-intent="groups">◉ گروه‌ها</button>
                        <button class="nh-intent domain" data-intent="governance">⚖ انتخابات و حکمرانی</button>
                        <button class="nh-intent domain" data-intent="najm_bahar">◈ نجم بهار</button>
                        <button class="nh-intent domain" data-intent="stock">▦ سهام و تأمین مالی</button>
                        <button class="nh-intent domain" data-intent="secretariat">▤ دبیرخانه</button>
                        <button class="nh-intent domain" data-intent="authority">⌘ اختیارها و واگذاری‌ها</button>
                    </div>
                </div>
            ...
            <div id="executiveReport" class="nh-report" aria-live="polite">
                <div class="nh-report-head">
                    <div>
                        <div id="reportTitle" class="nh-report-title">صبح مدیرکل</div>
                        <div id="reportMeta" class="nh-report-meta">در حال آماده‌سازی گزارش…</div>
                    </div>
                    <span id="reportAssessment" class="nh-assessment">در حال بررسی</span>
                </div>
                <div class="nh-report-body">
                    ...
                    <div id="detailSummaryWrap" class="nh-detail-summary-wrap" hidden>
                        <div class="nh-detail-summary-title">شاخص‌های همین گزارش</div>
                        <div id="detailSummaryCards" class="nh-summary"></div>
                    </div>
        ...
        <div id="free-chat" class="nh-pane">
            <div class="nh-free-chat-intro">
                <strong>گفت‌وگوی آزاد با نجم هدا</strong>
                <div class="text-muted small mt-1">این حالت گزارش مدیریتی را تغییر نمی‌دهد. برای پرسش عمومی، تحلیل، راهنمایی یا گفت‌وگو با agent انتخابی استفاده کنید.</div>
            </div>
        </div>
        ...
        <div class="nh-messages" id="chatMessages">
            <div class="nh-msg bot"><div class="nh-bubble"><strong>نجم هدا</strong><div class="mt-1">گزارش جاری در پنل وزارت نمایش داده می‌شود. اگر درباره آن سؤال داری، همین‌جا بپرس.</div></div></div>
        </div>

        <div class="nh-footer">
            <form id="chatForm" class="nh-form">
                @csrf
                <textarea id="messageInput" class="form-control" rows="1" maxlength="5000" placeholder="مثلاً: چرا این مورد را مهم تشخیص دادی؟"></textarea>
                <button class="btn nh-send px-4" type="submit" id="sendButton">ارسال</button>
            </form>
...
(()=>{
    ...
    const detailSummaryWrap=document.getElementById('detailSummaryWrap');
    const detailSummaryCards=document.getElementById('detailSummaryCards');
    const reportTitle=document.getElementById('reportTitle');
    const reportMeta=document.getElementById('reportMeta');
    const reportAssessment=document.getElementById('reportAssessment');
    ...
    let activePane='ministry';
    let lastIntent='morning_brief';
    ...
    windowSelect.addEventListener('change',()=>runMinistry({intent:lastIntent}));
