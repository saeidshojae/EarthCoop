...
<div class="container mx-auto px-4 py-6 max-w-7xl">
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
        ...
        <div class="flex flex-wrap gap-2">
            ...
            @can('inspect', $office)
                ...
                <a href="{{ route('secretariat.correspondence.create', ['office' => $office, 'direction' => 'outgoing']) }}"
                   class="inline-flex items-center justify-center gap-2 rounded-xl border border-sky-600 px-4 py-2.5 text-sky-700 hover:bg-sky-50">
                    <i class="fa-solid fa-paper-plane"></i> نامه صادره
                </a>
                <a href="{{ route('secretariat.correspondence.create', ['office' => $office, 'direction' => 'internal']) }}"
                   class="inline-flex items-center justify-center gap-2 rounded-xl border px-4 py-2.5 hover:bg-gray-50">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div class="rounded-2xl bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700 p-5"><div class="text-sm text-gray-500">پیش‌نویس‌های قابل مشاهده</div><div class="text-3xl font-bold mt-2">{{ $counts['draft'] }}</div></div>
        <div class="rounded-2xl bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700 p-5"><div class="text-sm text-gray-500">منتظر تأیید</div><div class="text-3xl font-bold mt-2">{{ $counts['pending_approval'] }}</div></div>
        <div class="rounded-2xl bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700 p-5"><div class="text-sm text-gray-500">رکوردهای رسمی</div><div class="text-3xl font-bold mt-2">{{ $counts['registered'] }}</div></div>
    </div>

    <form method="GET" action="{{ route('secretariat.index', $office) }}" class="rounded-2xl bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700 p-5 mb-6">
    ...
    <div class="rounded-2xl bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
        @forelse($records as $record)
            ...
            <a href="{{ $isCorrespondence ? route('secretariat.correspondence.show', [$office, $record]) : route('secretariat.records.show', [$office, $record]) }}" class="block border-b last:border-b-0 border-gray-100 dark:border-gray-700 p-4 hover:bg-gray-50 dark:hover:bg-gray-900/50">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                    <div class="min-w-0">
                        <div class="flex flex-wrap items-center gap-2 mb-1">
                            ...
                            @if(in_array($record->confidentiality, ['restricted', 'confidential'], true))<span class="text-xs rounded-full bg-amber-100 text-amber-800 px-2 py-1"><i class="fa-solid fa-lock ml-1"></i>{{ $record->confidentiality }}</span>@endif
                        </div>
    ...
    <p class="mt-3 text-xs text-gray-500">جست‌وجوی سریع حداکثر ۱۰۰ نتیجه مجاز را نمایش می‌دهد و هر نتیجه پیش از نمایش از Policy دبیرخانه عبور می‌کند.</p>
</div>
@endsection
