...
const installResidenceStyles = () => {
    ...
    style.textContent = `
        ...
        [data-location-proposal-toggle], .location-proposal-toggle { min-height: 44px; white-space: normal; text-align: start; }
...
const mountResidenceUx = (selector) => {
    ...
    if (currentLocationId && locationInput && !locationInput.value) {
    ...
    const renderPath = (items = selectedLabels()) => {
        ...
        items.forEach((item, index) => {
            ...
            node.className = item.proposal
                ? 'location-proposal badge bg-warning-subtle text-warning-emphasis'
