...
| LG-UAT-19 | City without urban region or neighborhood | With explicit `no_urban_region` + `no_neighborhood` claims, City exposes Street directly. A direct Street proposal must carry the same user's claims for that City, approval revalidates them, and the approved Street preserves claim IDs in provenance. | `ResidencePickerDeepHardeningTest` |
...
| LG-UAT-21 | Registration stopping rule | Registration must reach Neighborhood or the nearest structurally available base level, but Street/Alley/Complex/Building are optional. The UI tells the user that deeper address detail is not required. | `RegistrationPrimaryResidenceTest`, `CanonicalResidenceUiContractTest` |
| LG-UAT-22 | Pending structural proposal remains non-blocking | A valid structural proposal can be selected as pending exact residence without becoming canonical governance. Registration/profile continue from the approved anchor until review resolves the proposal. | `RegistrationPendingResidenceTest`, `ProfilePendingResidenceTest`, `PendingResidenceIntentTest` |
...
| LG-UAT-26 | Systemic election electorate | Official systemic election threshold, electorate, current-election surfaces, reminders, vacancy and responsibility flows use the canonical official topology and role=1 electorate contract. role=5 temporary active and Community membership do not activate or participate in systemic elections. | systemic election canonical/eligibility regressions |
