...
When a pending proposal is approved or merged, its active structural claims are re-anchored to the resolved canonical Location before residence/group reconciliation. Proposal support, approval, and merge revalidate the proposal's stored structural provenance; a rejected dependency cannot continue accumulating support or enter canonical data through merge. Merge also fails closed if the target already has the same active structural claim instead of silently duplicating structural identity.
...
- selecting or proposing a real Urban Region/Neighborhood fails closed if a contradictory `no_urban_region`/`no_neighborhood` claim is still submitted; the shared UI also clears that stale claim;
...
12. backend rejection of stale contradictory absence claims when a real Region/Neighborhood is selected or proposed;
13. rejected parent structural provenance blocks later proposal support and merge.
